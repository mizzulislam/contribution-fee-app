...
const adminNav = [
  ...
  { name: 'Kalkulator Split Bill', path: '/dashboard/split-bill' },
]
...
function getQuickActions(activeRole: Role | null, isSplitBillActive = true) {
  if (activeRole === 'super admin') {
    ...
    if (isSplitBillActive) {
      actions.push({ label: 'Kalkulator Split Bill', to: '/dashboard/split-bill' })
    }
    return actions
  }
...
describe('Dashboard Roles and Navigation Smoke Test', () => {
  ...
  describe('Dashboard Role Titles and Welcome Messages', () => {
    ...
    it('should return correct welcome message for each active role', () => {
      ...
      expect(getWelcomeMessage('admin')).toBe('Kelola operasional iuran, split bill, dan kebutuhan bersama.')
  ...
  describe('Dashboard Quick Actions Resolution', () => {
    it('should resolve super admin specific quick actions', () => {
      const actions = getQuickActions('super admin', false)
      ...
      const actionsActive = getQuickActions('super admin', true)
      expect(actionsActive.map(a => a.label)).toContain('Kalkulator Split Bill')
    })

    it('should resolve admin specific quick actions', () => {
      const actions = getQuickActions('admin', false)
      expect(actions.map(a => a.label)).toEqual([
        'Manajemen Iuran',
        'Verifikasi Bayar',
        'Catat Transaksi',
      ...
      const actionsActive = getQuickActions('admin', true)
      expect(actionsActive.map(a => a.label)).toContain('Kalkulator Split Bill')
    })

    it('should resolve user specific quick actions', () => {
      const actions = getQuickActions('user', false)
      expect(actions.map(a => a.label)).toEqual([
        'Pusat Pembayaran',
        'Kalender Kos',
        'Pusat Informasi',
      ...
      const actionsActive = getQuickActions('user', true)
      expect(actionsActive.map(a => a.label)).toContain('Kalkulator Split Bill')
    })
  })
  ...
  describe('Cleanliness and Absence of Dummy Data / Placeholders', () => {
    it('should not contain TODOs, placeholder names, or lorem ipsum', () => {
      const allTextContents = [
        ...
        ...getQuickActions('super admin', true).map(a => a.label),
        ...getQuickActions('admin', true).map(a => a.label),
        ...getQuickActions('user', true).map(a => a.label),
      ]
