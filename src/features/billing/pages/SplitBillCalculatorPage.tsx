    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-slate-950 to-emerald-950 flex flex-col justify-center items-center py-12 px-4 select-none">
        ...
        <div className="w-full max-w-md flex flex-col space-y-6 animate-in fade-in duration-500">
          ...
          <div id="premium-receipt-card" className="bg-white rounded-[24px] text-gray-900 shadow-[0_20px_50px_rgba(0,0,0,0.06)] border border-gray-200 relative overflow-hidden flex flex-col p-6 space-y-6">
            
            <div className="flex justify-between items-center border-b border-gray-150 pb-4">
              <div className="flex items-center gap-2">
                <Receipt className="w-5 h-5 text-emerald-600" />
                <h2 className="text-base sm:text-lg font-extrabold tracking-wide text-gray-955">Ringkasan Pembagian</h2>
              </div>
              <span className="inline-flex items-center justify-center text-[10px] font-black uppercase tracking-wider bg-emerald-50 text-emerald-700 px-3 py-1.5 rounded-lg border border-emerald-250/20 select-none">
            ...
            <div className="space-y-3.5 text-sm">
              ...
              {sharedCustomAdjustments.map((adj: any) => {
                ...
                const isDiscount = adj.type === 'discount'
                return (
                  <div key={adj.id} className="flex justify-between items-center text-gray-600 animate-in fade-in duration-200">
                    <span className="flex items-center gap-2">
                      {isDiscount ? (
                        <Tag className="w-4 h-4 text-amber-600 shrink-0" />
                      ) : (
              ...
              <div className="flex justify-between items-baseline border-t border-gray-150 pt-4">
                <span className="text-xs font-extrabold uppercase tracking-wider text-gray-500">Total Akhir</span>
            ...
            <div className="border-t border-dashed border-gray-200 my-2 select-none pointer-events-none" />

            <div className="flex-1 flex flex-col space-y-3">
              <span className="text-[10px] font-black uppercase tracking-widest text-emerald-600 block">Rincian Pembayaran:</span>
              <div className="space-y-2.5">
                {shares.map((share: any) => (
                  <div key={share.id} className="flex justify-between items-center bg-gray-50 p-3.5 rounded-xl border border-gray-200 hover:bg-gray-100 hover:border-gray-300 transition-all duration-200">
                    <div className="min-w-0 pr-3 flex-grow">
                      <p className="font-extrabold text-xs sm:text-sm text-gray-900 flex items-center gap-1.5">
                        {share.name}
                        {share.phone && (
                          <span className="text-[9px] text-emerald-700 font-bold bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-200/50">
                            {share.phone}
                          </span>
                        )}
                      </p>
                    ...
                    <span className="font-black text-xs sm:text-base text-gray-950">{formatCurrency(share.total)}</span>
                  </div>
                ))}
              </div>
            </div>
            
            <div className="bg-emerald-50/50 p-3 rounded-xl border border-emerald-100 flex items-start gap-2.5 text-[10.5px] text-emerald-800 leading-relaxed no-export">
    ...
    <div className="space-y-6 w-full animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
        ...
        <div className="lg:col-span-5 flex flex-col">
          <div id="premium-receipt-card" className={`bg-white rounded-[24px] text-gray-900 shadow-[0_20px_50px_rgba(0,0,0,0.06)] border border-gray-200 relative overflow-hidden flex flex-col ${isExporting ? 'h-auto' : 'h-full'}`}>
            
            {/* Card Title Header (aligned with left tab bar) */}
            <div className="h-[60px] flex justify-between items-center px-6 border-b border-gray-150 shrink-0">
              <div className="flex items-center gap-2">
                <Receipt className="w-5 h-5 text-emerald-600" />
                <h2 className="text-base sm:text-lg font-extrabold tracking-wide text-gray-950">
                  Ringkasan Pembagian
                </h2>
              </div>
              <span className="inline-flex items-center justify-center text-[10px] font-black uppercase tracking-wider bg-emerald-50 text-emerald-700 px-3 py-1.5 rounded-lg border border-emerald-250/20 select-none">
            ...
            <div className="p-6 space-y-6 flex-1 flex flex-col">
              ...
              <div className="flex-1 flex flex-col space-y-3 min-h-[220px]">
                ...
                <div className={isExporting ? "space-y-2.5" : "flex-1 space-y-2.5 overflow-y-auto pr-1 max-h-[250px] lg:max-h-[300px] [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-white/10 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-white/20"}>
                  {shares.length === 0 ? (
                    ...
                    shares.map(share => (
                      <div key={share.id} className="flex justify-between items-center bg-gray-50 p-3.5 rounded-xl border border-gray-200 hover:bg-gray-100 hover:border-gray-300 transition-all duration-200 animate-in fade-in duration-300">
                        <div className="min-w-0 pr-3 flex-grow">
                          <p className="font-extrabold text-xs sm:text-sm truncate text-gray-900 flex items-center gap-1.5">
                            {share.name}
                            {share.phone && (
                              <span className="text-[9px] text-emerald-700 font-bold bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-200/50">
                                {share.phone}
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <span className="font-black text-xs sm:text-base text-gray-950 text-right">{formatCurrency(share.total)}</span>
                          <button
                            type="button"
                            onClick={() => handleSendIndividualWhatsApp(share)}
                            className="p-1.5 text-emerald-600 hover:text-white hover:bg-emerald-600 rounded-lg transition-all border border-emerald-250 hover:border-emerald-600 cursor-pointer flex items-center justify-center no-export"
              ...
              {/* Info Message */}
              <div className="bg-emerald-50/50 p-3 rounded-xl border border-emerald-100 flex items-start gap-2.5 text-[10.5px] text-emerald-800 leading-relaxed shadow-inner no-export">
                <Info className="w-4 h-4 text-emerald-600 shrink-0 mt-0.5" />
                <p>Pajak, biaya layanan, dan diskon dialokasikan secara proporsional sesuai porsi belanja masing-masing peserta.</p>
              </div>
